...
public final class LauncherConfigurationExample {
 ...
 public static void main(String[] args) throws Exception { if(args.length!=1) throw new IllegalArgumentException("Usage: LauncherConfigurationExample <job.conf>"); ReadonlyConfig config=new HoconConfigLoader().load(Paths.get(args[0])); JobResult result=new LocalJobExecutor().execute(new JobPlanner(FactoryRegistry.discover()).plan(toJobDefinition(config))); System.out.println("Job completed: read="+result.readRecords()+", written="+result.writtenRecords()+", failed="+result.failedRecords()+", elapsedMs="+result.elapsedMillis()+", recordsPerSecond="+result.recordsPerSecond()); }
 public static JobDefinition toJobDefinition(ReadonlyConfig config) { ConfigValidator.strict().validate(config,JOB_RULE).throwIfInvalid(); return new JobDefinition(config.get(NAME),Boundedness.BOUNDED,config.get(BATCH_SIZE),config.get(SOURCE_TYPE),ReadonlyConfig.fromMap(config.get(SOURCE_OPTIONS)),config.get(SINK_TYPE),ReadonlyConfig.fromMap(config.get(SINK_OPTIONS))); }
}
