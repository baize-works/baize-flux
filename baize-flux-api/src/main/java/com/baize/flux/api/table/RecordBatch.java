...
public final class RecordBatch<T> {
    ...
    private RecordBatch(java.util.List<T> records, boolean endOfInput) {
        this.records = java.util.Collections.unmodifiableList(new java.util.ArrayList<>(records));
        this.endOfInput = endOfInput;
    }

    public static <T> RecordBatch<T> of(java.util.List<T> records) {
        return new RecordBatch<T>(java.util.Objects.requireNonNull(records, "records"), false);
    }

    @SuppressWarnings("unchecked")
    public static <T> RecordBatch<T> endOfInput() {
        return (RecordBatch<T>) END;
    }
