    ...
    private <SplitT extends SourceSplit> ExecutionPlan createPlan(PreparedJob job, PreparedSource<SplitT> preparedSource) throws Exception {
        ...
        for (Map.Entry<String, List<SplitT>> entry : byDataSet.entrySet()) {
            ...
            for (int i = 0; i < assignments.size(); i++)
                sources.add(new SourceTaskPlan<SplitT>(new TaskId(pipelineId, com.baize.flux.framework.execution.TaskType.SOURCE, i, assignments.size()), preparedSource, assignments.get(i), config.getBatchSize(), provider));
            ...
            for (int i = 0; i < sinkCount; i++)
                sinkPlans.add(new SinkTaskPlan(new TaskId(pipelineId, com.baize.flux.framework.execution.TaskType.SINK, i, sinkCount), sinks.get(i)));
